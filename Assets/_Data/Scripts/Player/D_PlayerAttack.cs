...
public class D_PlayerAttack : Attack
{
    [SerializeField] private PlayerSwingVisual playerSwingVisual;
    ...
    private void Start()
    {
        ...
        playerSwingVisual = GameObject.Find("PlayerAttackDir").GetComponentInChildren<PlayerSwingVisual>();
    }
    private enum State
    {
        idleAttack,
        ...
        switch (state)
        {
            case State.idleAttack:
                if (player.attackIndex == 1)
                {
                    ...
                    playerSwingVisual.SwingAnimation("Swing");
            ...
            case State.entryAttack:
                ...
                if (player.comboDurantion > 0)
                {
                    ...
                    if (player.attackIndex == 2)
                    {
                        ...
                        playerSwingVisual.SwingAnimation("Attack1");
            ...
            case State.comboAttack:
                if (player.comboDurantion > 0)
                {
                    ...
                    if (player.attackIndex == 0)
                    {
                        ...
                        playerSwingVisual.SwingAnimation("Attack2");
    ...
    private void OnDrawGizmos()
    {
        ...
        Gizmos.DrawWireSphere(hitbox1.position, 2f);
    }
    ...
    public override void MeleeAttack(int attackIndx)
    {
        ...
        else if (attackIndx == 0)
        {
            ...
            Collider2D[] hit = Physics2D.OverlapCircleAll(hitbox1.position, 1.5f, enemyLayer);
            for (int i = 0; i < hit.Length; i++)
            {
                ...
                {
                    Enemy enemy = hit[i].gameObject.GetComponent<Enemy>();
                    player.DealDamage(enemy, attackDmg * 2f);
                    Knockback(hit[i].transform);
                    if (!enemy.IsAlive())
                    {
                        enemy.PlayAnimationAndDead();
